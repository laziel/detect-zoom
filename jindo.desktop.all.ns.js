...
브라우저 버전이 IE5.5 이하
				 ...
				 */
				if (/\bMSIE\s([0-9]+(\.[0-9]+)*);/.test(_j_ag) && parseFloat(RegExp.$1) < 6) {
...
jindo.$A.prototype.map = function(fCallback, oThis) {
	...
	function mapBody(fpEach){
		return function(fCallback, oThis) {
			...
			function f(v,i,a) {
				try {
					returnArr.push(fCallback.apply(oThis||that, _toArray(arguments)));
				} catch(e) {
...
테스트를 위해 우선 적용가능한 설정 객체가 존재하면 적용
	...
	switch (_opt.type) {
		...
		case "flash":
			//-@@$Ajax.flash-@@//
			if(!___ajax.SWFRequest) throw new ___error(jindoName+'.$Ajax.SWFRequest'+___except.REQUIRE_AJAX, "$Ajax");
		...
		case "iframe":
			//-@@$Ajax.iframe-@@//
			if(!___ajax.FrameRequest) throw new ___error(jindoName+'.$Ajax.FrameRequest'+___except.REQUIRE_AJAX, "$Ajax");
	...
	if(isIE){
		return function(){
			...
			if (this._request.readyState == 4) {
				try {
						if ((!bSuccess) && ___jindo.isFunction(this._options.onerror)){
...
jindo.$Ajax.SWFRequest = klass({
 ...
 콜백코드는 넣었지만, 아직 SWF에서 응답헤더 지원 안함
	...
	send : function(data) {
		...
		window["__"+jindoName+"_callback"][info.id] = function(success, data){
			try {
	...
	abort : function(){
		...
		if(this._current_callback_id){
			window["__"+jindoName+"_callback"][this._current_callback_id] = function(){
				delete window["__"+jindoName+"_callback"][info.id];
			}
		}
	}
...
jindo.$Ajax.SWFRequest.write = function(swf_path) {
	...
	var activeCallback = jindoName+".$Ajax.SWFRequest.loaded";
...
jindo.$Ajax.FrameRequest = klass({
	...
	send : function(data) {
		...
		window["__"+jindoName+"_callback"][info.id] = function(id, data, header){
			try {
...
function _ie_check_scroll(ele,e){
    if(document.body.componentFromPoint&&parseInt(_j_ag.match(/(?:MSIE) ([0-9.]+)/)[1],10) == 8){
        _ie_check_scroll = function(ele,e){
            return !/HTMLGenericElement/.test(ele+"") && /(scrollbar|outside)/.test(ele.componentFromPoint(e.clientX, e.clientY));
        }
    }else{
        _ie_check_scroll = function(ele,e){
...
jindo.$Element.prototype.css = function(sName, sValue) {
    ...
    switch(oArgs+"") {
        case 's4str':
        case 's4num':
            sName = cssNameToJavaScriptName(sName);
            var obj = {};
            obj[sName] = sValue;
            sName = obj;
            break;
        ...
        case 'g':
            ...
            if ((!_JINDO_IS_OP && window.getComputedStyle) && (sName=="padding"||sName=="margin")) {
    ...
    for(var k in sName) {
        if(sName.hasOwnProperty(k)){
            ...
            if((_JINDO_IS_FF||_JINDO_IS_OP)&&( k =="backgroundPositionX" || k == "backgroundPositionY")){
                ...
                this._setCss(e, cssNameToJavaScriptName(k), v);
            }
        }
    }
...
jindo.$Element.eventManager = (function(){
	...
	@param {Function} fpCallback 이벤트 콜백함수
        ...
        revisionCallback : function(sType, sEvent, realEvent, fpCallback){
            if((document.addEventListener||_JINDO_IS_IE&&(sType=="delegate"))&&(realEvent=="mouseenter"||realEvent=="mouseleave")) 
            // ||(_JINDO_IS_IE&&(sType=="delegate")&&(realEvent=="mouseenter"||realEvent=="mouseleave")))
               {
 ...
 * IE9인 경우도 DOMMouseScroll이 동작하지 않음.
                    ...
                    }else if (sEvent == "mouseenter" && (!_JINDO_IS_IE||sType=="delegate")){
                        sEvent = "mouseover";
                    }else if (sEvent == "mouseleave" && (!_JINDO_IS_IE||sType=="delegate")){
                        sEvent = "mouseout";
                        ...
                        if(document.body.style.animationName !== undefined){
                            sEvent = sEvent;
                        }else if(document.body.style.WebkitAnimationName !== undefined){
...
jindo.$Element._getTransition = function(){
    var hasTransition = false , sTransitionName = "", sTransitionEndName="";
    
    if (typeof document.body.style.transition !== "undefined") {
        hasTransition = true;
        sTransitionName = "transition";
        sTransitionEndName = "transitionend";
    }
    /*
...
jindo.$Element.prototype.appear = function(duration, callback) {
    ...
    if (oTransition.hasTransition) {
        jindo.$Element.prototype.appear = function(duration, callback) {
            ...
            var endName = oTransition.endName;
            var bindFunc = function(){
                ...
                ele.removeEventListener(endName, arguments.callee , false );
            };
            if(!this.visible()){
    ...
    if (oTransition.hasTransition) {
        jindo.$Element.prototype.disappear = function(duration, callback) {
            ...
            var bindFunc = function(){
                ...
                ele.style.opacity = '';
                callback.call(self,self);
                this.removeEventListener(endName, arguments.callee , false );
            };
            var ele = this._element;
            ele.addEventListener( endName, bindFunc , false );
...
/**
	...
	@return {Object} 가로 위치는 left, 세로위치는 top 라는 키값으로 리턴된다.
		...
		@return {Number} .height 수직 스크롤바의 세로 위치. (단위 px)
	@since 1.3.5
	@example
...
jindo.$Template.addEngine("default", function(sTemplate){
	...
	if(i < 2){
	    ...
	    // console.log("return '" + tpl[0].replace(/'/g, "\\'") + "';");
	    // return new Function("return '" + tpl[0].replace(/'/g, "\\'") + "';");
...
jindo.$Window.prototype.resizeTo = function(nWidth, nHeight) {
	//-@@$Window.resizeTo-@@//
	var oArgs = g_checkVarType(arguments, {
		'4num' : ['nWidth:'+tNumeric,'nHeight:'+tNumeric]
	},"$Window#resizeTo");
...
jindo.$Window.prototype.moveTo = function(nLeft, nTop) {
	//-@@$Window.moveTo-@@//
	var oArgs = g_checkVarType(arguments, {
		'4num' : ['nLeft:'+tNumeric,'nTop:'+tNumeric]
	},"$Window#moveTo");
